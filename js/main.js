...
const Main = (() => {
  ...
  function init() {
    ...
    try { if (localStorage.getItem('pandataria-textsize') === 'large') document.documentElement.classList.add('text-large'); } catch (e) {}
    $('btn-textsize').onclick = () => {
      const large = document.documentElement.classList.toggle('text-large');
      try { localStorage.setItem('pandataria-textsize', large ? 'large' : 'normal'); } catch (e) {}
    };
  ...
  /* Gli eroi selezionabili al setup sono i due che partono da Scauri: Gaetano e Claudia.
     Ciro è `locked: true` — si unisce il 29 agosto, giocando, se se ne guadagna la fiducia. */
  ...
  function renderSetup() {
    ...
    for (const h of selectableHeroes()) {
      ...
      card.querySelector('[data-act="toggle"]').onclick = e => {
        ...
        if (!selection[h.id].selected && count >= selectableHeroes().length) return;
  ...
  function updateSetupBar() {
    const count = Object.values(selection).filter(s => s.selected).length;
    $('setup-count').textContent = `In vacanza: ${count} / ${selectableHeroes().length}` +
      (count === 0 ? ' (minimo 1)' : count === 1 ? ' — 🌊 uno solo al timone: l\'altro lo gestisce il Narratore' : '');
  ...
  function showCharDetail(h) {
    ...
    const pickBtn = h.locked
      ? `<button class="btn" disabled>🔒 Si unisce il 29 agosto, se vi guadagnate la sua fiducia</button>`
    ...
    if (!h.locked) {
      $('btn-char-pick').onclick = () => {
        const count = Object.values(selection).filter(s => s.selected).length;
        if (!selection[h.id].selected && count < selectableHeroes().length) selection[h.id].selected = true;
        $('modal-char').classList.add('hidden');
        renderSetup();
      };
    }
