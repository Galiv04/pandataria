...
const NERO_VOLUTO = {
  relitto:    'il buco nella stiva a quarantacinque metri: la torcia ci entra e non torna',
  panopticon: "le celle socchiuse: un vano con niente dietro E' nero pieno",
};
...
/* E i flag che il codice scrive, letti dai sorgenti invece che tenuti a mano. Una lista
   scritta a mano e' una lista che a un certo punto non e' aggiornata: nella Casa che non
   Finisce l'aggiunta di un'impresa sul flag `solo` — che il motore imposta alla riga
   quattro di newGame(), `flags: solo ? { solo: true } : {}` — ha fatto fallire il collaudo
   dicendo che quel flag non lo imposta nessuno. Il controllo aveva ragione sulla sua lista
   e torto sul gioco. I tre modi sono: assegnamento, incremento, inizializzazione. */
for (const f of readdirSync(join(root, 'js')).filter(f => f.endsWith('.js') && f !== 'campaign.js')) {
  const src = readFileSync(join(root, 'js', f), 'utf8');
  for (const m of src.matchAll(/flags\s*\.\s*([A-Za-z_$][\w$]*)\s*(?:=[^=]|\+\+|--|\+=)/g)) FLAG_DEL_MOTORE.add(m[1]);
  for (const m of src.matchAll(/flags\s*\[\s*['"]([^'"]+)['"]\s*\]\s*(?:=[^=]|\+\+|--|\+=)/g)) FLAG_DEL_MOTORE.add(m[1]);
  for (const m of src.matchAll(/flags\s*:\s*[^;\n]{0,240}/g))
    for (const k of m[0].matchAll(/([A-Za-z_$][\w$]*)\s*:\s*(?:true|false|0|1)\b/g)) FLAG_DEL_MOTORE.add(k[1]);
}
...
function testFlagRichiestiMaiImpostati() {
  ...
  /* I FLAG DEI MINIGIOCHI. La scansione qui sopra trova solo le chiavi LETTERALI, e
     minigames.js scrive `G.flags[cfg.extraFlag || 'ha_visto_giu'] = true`: una chiave
     calcolata, che nessuna espressione regolare puo vedere. Risultato: un flag assegnato
     davvero dal minigioco veniva dichiarato «mai impostato», e la prima scelta che lo
     usava passava per contenuto irraggiungibile. Falso positivo, ed e il tipo peggiore —
     manda a cercare un difetto che non c'e e insegna a diffidare del rosso. */
  ...
  impostati.add('ha_visto_giu');   // il predefinito di minigames.js quando extraFlag manca
